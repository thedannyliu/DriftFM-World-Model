...
METRICS_DIR=${RUNTIME_ROOT}/results/${EXPERIMENT_TAG}-seed${SEED}-${CHECKPOINT_KIND}
LOG_DIR=${RUNTIME_ROOT}/logs/variant-eval/${EXPERIMENT_TAG}-${CHECKPOINT_KIND}-${TIMESTAMP}
MARKER=${ASSET_ROOT}/checkpoints/experiments/eval-${EXPERIMENT_TAG}-seed${SEED}-${CHECKPOINT_KIND}.json
...
echo "[variant-eval] tag=${EXPERIMENT_TAG} checkpoint=${CHECKPOINT_KIND} videos=${NUM_VIDEOS}"
...
"${PYTHON_BIN}" "${REPO_ROOT}/company/summarize_eval.py" \
    --variant-dir "${METRICS_DIR}" | tee "${MARKER}"
