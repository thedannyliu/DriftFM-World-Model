...
| D2e | planned (runner ready) | Endpoint compatibility used the wrong conversion from endpoint prediction to mean velocity: `G(x_t)-x_t` must be divided by remaining path time `1-t`. | Evaluate existing checkpoints without retraining using `x_r=x_t+(r-t)/(1-t)(G(x_t)-x_t)` via `company/run_endpoint_normalized_eval.sh`; NFE1 is algebraically unchanged. | If best/replay checkpoints restore NFE2/4 near or below NFE1 while the old sampler fails, the parameterization error is causal. Failure would implicate endpoint prediction on intermediate states or the distributional objective. |
