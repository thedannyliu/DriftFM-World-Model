...
| D2b | planned (runner ready) | Intermediate target marginal is under-sampled: implementation uses 16 source particles but one noisy positive although the design specifies K positives. | `company/run_transport_repair.sh node-a`: 10k with 16 intermediate positives while endpoint replay retains one positive, then latest/best NFE1/2/4 evaluation. | If K positives improve NFE scaling without sacrificing NFE1, support. |
