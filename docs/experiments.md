...
| CA1/CA2 | complete training; uniform rollout complete, replay .50 pending | Drift Flow time-sampling ablation: uniform; endpoint-replay ablation: 0.50; seed 1, 10k each | `company/run_overnight.sh` | one independent 4xH100 node each after matched runs | official step 1180500 -> `driftflow-uniform_seed1` / `driftflow-replay50_seed1` | uniform `ralk43lj`; replay .50 `oirs7ece` | Uniform full LPIPS latest NFE1/2/4 `(.03032,.52813,.69085)` and best `(.00631,.71120,.86651)`. Replay .50 rollout pending. | Uniform sampling trades 29% worse latest NFE1 for only 10%/9% better NFE2/4 versus the original 10k DFM; its best checkpoint reproduces the collapse. General continuous-time coverage is insufficient, while exact-grid coverage remains open. |
...
| --- | --- | --- | --- | --- |
| D2a | planned (runner ready) | Local maps fail because continuous sigmoid sampling never trains the exact boundary intervals used by NFE2/4: `(0,.5)`, `(0,.25)`, and `(.75,1)`. | `company/run_transport_repair.sh node-b`: 10k with 25% inference-grid replay, then latest/best NFE1/2/4 evaluation. | If grid replay repairs NFE scaling without destroying NFE1, support; otherwise exact-grid coverage is insufficient. |
...
| D2c | planned (runner ready) | Local maps work on interpolant sources but composed predictions leave the training support. | `company/run_transport_audit.sh`: compare paired teacher-forced transitions and free composition on identical NFE1/2/4 grids using the same source particles. | Teacher-forced progress near one with much worse free MSE supports rollout/self-forcing or semigroup training; failure of both rejects this as the first fix. |
| D2d | planned (runner ready) | The post-trained network remains an endpoint predictor and does not use the time pair as a mean-velocity model. | On the same teacher-forced source, compare the correct interval velocity with the `(0,1)` velocity; report cosine, relative norm, mean absolute difference, displacement progress, and orthogonal error. | Near-endpoint velocity with progress proportional to interval length supports an explicit velocity head or endpoint-preserving residual adapter; time-sensitive correct progress rejects this mechanism. |
